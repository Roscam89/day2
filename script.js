...
const data = `Game 1: 2 red, 2 green; 6 red, 3 green; 2 red, 1 green, 2 blue; 1 red
...
const red = dataArr.map((e) => e.match(/\d+ red/g).join(","));
const redNr = red.map((e) => e.match(/\d+/g));
const green = dataArr.map((e) => e.match(/\d+ green/g).join(","));
const greenNr = green.map((e) => e.match(/\d+/g));
const blue = dataArr.map((e) => e.match(/\d+ blue/g).join(","));
const blueNr = blue.map((e) => e.match(/\d+/g));
...
redNr.forEach((e, i) => {
  ...
  const redNrS = e.map((e) => e * 1).reduce((a, b) => a + b);
  if (redNrS <= 12) {
    gameIdArr.push(gameId);

    console.log("Game:", gameId, "possible");
  } else console.log("Game:", i + 1, "IMPO");
});

greenNr.forEach((e, i) => {
  let gameId = i + 1;

  const greenNrS = e.map((e) => e * 1).reduce((a, b) => a + b);
  if (greenNrS <= 13) {
    gameIdArr.push(gameId);
...
blueNr.forEach((e, i) => {
  ...
  const blueNrS = e.map((e) => e * 1).reduce((a, b) => a + b);
  if (blueNrS <= 14) {
    gameIdArr.push(gameId);
    console.log("Game:", gameId, "possible");
  } else console.log("Game:", i + 1, "IMPO");
});
console.log(gameIdArr.reduce((a, b) => a + b));
